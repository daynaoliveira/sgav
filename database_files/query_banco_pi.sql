...
CREATE TABLE IF NOT EXISTS user_types(
	id_user_type INT PRIMARY KEY UNIQUE NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS users(
    ...
    created_at TIMESTAMP NOT NULL,
    updated_at DATETIME DEFAULT NULL,
	CONSTRAINT fk_employee FOREIGN KEY(id_employee) REFERENCES employess(id_employee) ON DELETE CASCADE ON UPDATE CASCADE,
	CONSTRAINT fk_user_type FOREIGN KEY(id_user_type) REFERENCES user_types(id_user_type) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS cars(
	id_cars INT PRIMARY KEY UNIQUE NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS cash_flow(
	id_cash_flow INT PRIMARY KEY UNIQUE NOT NULL AUTO_INCREMENT COMMENT 'Fluxo do caixa',
    id_user INT NOT NULL,
    id_car INT COMMENT 'ID do carro comprado',
    id_rental_car INT COMMENT 'ID do carro alugado',
    service VARCHAR(100) NOT NULL COMMENT 'Serviço que gerou o movimento no caixa',
    i_o BOOLEAN DEFAULT 1 COMMENT 'Entrada/saída de dinheiro no caixa [entrada = 1 | saída = 0]',
    payment_method VARCHAR(50) NOT NULL,
    installments INT(2) COMMENT 'Parcelas da compra',
    pay_date DATE COMMENT 'Data de pagamento das parcelas',
    due_date DATE COMMENT 'Data de vencimento das parcelas',
    paid_in DATE COMMENT 'Pago em',
    price DOUBLE(10, 2) COMMENT 'Preço do serviço',
    price_interest DOUBLE (10, 2) COMMENT 'Preço do serviço com juro de atraso',
    CONSTRAINT fk_user FOREIGN KEY(id_user) REFERENCES users(id_user) ON DELETE CASCADE ON UPDATE CASCADE,
	CONSTRAINT fk_car FOREIGN KEY(id_car) REFERENCES cars(id_car) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_rental_car FOREIGN KEY(id_rental_car) REFERENCES rental_cars(id_rental_car) ON DELETE CASCADE ON UPDATE CASCADE
);
